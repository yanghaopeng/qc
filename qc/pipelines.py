...
class QcMysqlPipeline(object):
    def open_spider(self, spider):
        self.con = pymysql.connect(host="localhost", port=3306, database="qiancheng", user="root", password="mysql", charset="utf8")
        self.cur = self.con.cursor()

    def process_item(self, item, spider):
        sql = ("insert into qiancheng(source, utcTime, workName, company, workPosition, salary, publishTime, content, contact)"\
               "values (%s, %s, %s, %s, %s, %s, %s, %s, %s)")
        list_item = [item['source'], item['utc_time'], item['work_name'], item['company'],item['work_position'],
                     item['salary'], item['publishTime'], item['content'], item['contact']]
